...
void verifica_aumenta(Fila* f){
    if(f->qtde == f->tam){
        f->tam = f->tam * 2;
        TipoElemento* vetor_novo = (TipoElemento*) calloc(f->tam, sizeof(TipoElemento));
        for (int i = 0; i < f->tam/2; ++i) {
            vetor_novo[i] = f->vetor[i];
        }
        free(f->vetor);
        f->vetor = vetor_novo;
    }
...
void verifica_diminui(Fila* f){
    if(f->qtde < ((int)(f->tam * 0.25))){
        f->tam = (int)f->tam * 0.5;
        TipoElemento* vetor_novo = (TipoElemento*) calloc(f->tam, sizeof(int));
